...
const resolvers = {
  Query: {
    ...
    allAuthors: () => {
      ...
      for (let book of books) {
        bookCountByAuthor[book.author] =
          (bookCountByAuthor[book.author] || 0) + 1;
      }
  ...
  Mutation: {
    addBook(root, data) {
      ...
      books.push(newBook);
      console.log(books);
      // if the author is not yet regitereed in the server, add him:
      if (!authors.find((author) => author.name === data.author))
        authors.push({ name: data.author, id: uuid() });
      return newBook;
    },
    editAuthor(root, { name, setBornTo: born }) {
