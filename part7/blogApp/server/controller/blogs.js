    ...
    try {
      ...
      if (!toDelete.author.equals(user._id)) {
        return response.status(401).json({
          error: "deletion is only possible if logged-in as the author",
        });
      }
...
blogsRouter.put("/:id", authenticateUser, async (request, response, next) => {
  try {
    ...
    if (request.user.id.toString() !== blog.author.toString()) {
      return response.status(401).json({
        error: "updating is only possible if logged-in as the author",
      });
    }
...
blogsRouter.post("/:id/comments", async (request, response, next) => {
  try {
    ...
    await blog.save();
    const comment = new Comment({ content: request.body, blog: blog.id });
